...
use Morpheus\Shared\Classes\Hook;
use WP_Post;
...
trait HasHooks
{
    ...
    public static function forceSavePost(int $postId, bool $update = false): void
    {
        $post = WP_Post::get_instance($postId);
        $sendToHermes = new PostOnSave;
        $sendToHermes($post->ID, $post, $update);
    }
}
